...
class DashboardService
{
    ...
    public function getLabelSummaryByRange(?Carbon $start = null, ?Carbon $end = null): array
    {
        ...
        $promoted = LabelPromoted::query()
            ...
            ->when(isset($start), fn($query) => $query->where('match.match_time', '>=', crown_time($start)->toISOString()))
            ->when(isset($end), fn($query) => $query->where('match.match_time', '<', crown_time($end)->toISOString()))
